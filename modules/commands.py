...
class Commands:
    ...
    async def faculty_roles(ctx: SlashInteraction):
        ...
        if author_permission.administrator:
            ...
            await ctx.reply(content='You need to an administrator to do that', ephemeral=True, delete_after=1)
    ...
    @staticmethod
    async def edit_post(ctx: SlashInteraction, channel_id: int, message_id: int, message: str):
        author_permission: Permissions = ctx.author.permissions_in(ctx.channel)
        if author_permission.administrator:
